...
type Role struct {
	...
	RoleMenus RoleMenus `json:"role_menus"`                            // 角色菜单列表
}
 ...
 * 填充角色菜单
 *
 * param: map[string]RoleMenus mActions
 * return: Roles
 */
func (a Roles) FillRoleMenus(mRoleMenus map[string]RoleMenus) Roles {
	for _, item := range a {
		if v, ok := mRoleMenus[item.ID]; ok {
			item.RoleMenus = v
		} else {
			item.RoleMenus = make(RoleMenus, 0)
		}
	}
	return a
}
...
func (a RoleMenus) ToMap() map[string]*RoleMenu {
	...
	for _, item := range a {
		m[item.MenuID+"-"+item.MenuID] = item
	}
	return m
}
