...
//TODO 这里的CasbinAdapter是实现了casbin中的Adapter接口，即CasbinAdapter实现了LoadPolicy，SavePolicy，AddPolicy， RemovePolicy方法，
//TODO 并且通过在LoadPolicy将用户权限和角色权限从数据库中进行加载。
// CasbinAdapter casbin适配器
type CasbinAdapter struct {
	RoleModel         model.IRole
	RoleMenuModel     model.IRoleMenu
	MenuResourceModel model.IMenu
...
func (a *CasbinAdapter) loadRolePolicy(ctx context.Context, m casbinModel.Model) error {
	...
	menuResourceResult, err := a.MenuResourceModel.Query(ctx, schema.MenuQueryParam{Level: 3})
	if err != nil {
		return err
	}
	mMenuResources := menuResourceResult.Data.ToParentIDMap()

	for _, item := range roleResult.Data {
		mcache := make(map[string]struct{})
		if rms, ok := mRoleMenus[item.ID]; ok {
			for _, MenuID := range rms.ToMenuIDs() {
				if mr, ok := mMenuResources[MenuID]; ok {
					//这种菜单不是按钮类型
...
/**
 * 加载用户策略(g,user_id,role_id)就是指明用户属于哪些角色
 *
 * param: context.Context   ctx
 * param: casbinModel.Model m
 * return: error
 */
